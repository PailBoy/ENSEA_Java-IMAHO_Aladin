...
public class DynamicSprite extends SolidSprite {
    ...
    private long invincibilityDuration = 2000; // 2 secondes
    ...
    public void takeDamage(int damage) {
        ...
        // Vérifier si le personnage est invincible
        if (invincible || (currentTime - lastDamageTime) < invincibilityDuration) {
            System.out.println("Le personnage est invincible !");
            return;
        }

        // Infliger des dégâts
        setCurrentHealth(currentHealth - damage);
        System.out.println("Le personnage subit " + damage + " points de dégâts. Santé actuelle : " + currentHealth);

        // Activer l'invincibilité
        invincible = true;
        lastDamageTime = currentTime;

        // Désactiver l'invincibilité après un délai
        new Thread(() -> {
            try {
            ...
            invincible = false;
            System.out.println("L'invincibilité a expiré.");
        }).start();
    }
